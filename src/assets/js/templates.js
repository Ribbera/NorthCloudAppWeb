(function($) {
    ...
    // Indicator.
    const link = $(
        "#theme-navbar .nav-bottom .links .link:not(.has-dropdown-menu)"
    );
    const ind = $("#theme-navbar .nav-bottom .links .indicator");
    const linkWidth = $(
        "#theme-navbar .nav-bottom .links .link:nth-child(2)"
    ).width();
    ...
    $("#theme-navbar .link.has-dropdown-menu > a").on("click", function() {
        $(this)
            .parent()
            .toggleClass("open-dropdown-menu");
    });
    ...
    $(document).mouseup(function(e) {
        ...
        if (
            !userDropMenu.is(e.target) &&
            userDropMenu.has(e.target).length === 0
        ) {
